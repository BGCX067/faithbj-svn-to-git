...
import com.faithbj.shop.entity.MemberRank;
...
import com.faithbj.shop.entity.Deposit.DepositType;
...
import com.faithbj.shop.entity.Payment.PaymentStatus;
import com.faithbj.shop.entity.Payment.PaymentType;
import com.faithbj.shop.entity.Product.WeightUnit;
import com.faithbj.shop.entity.Refund.RefundType;
...
import com.faithbj.shop.util.ExcelOper;
...
public class OrderAction extends BaseAdminAction {
	...
	// 导出
	public void excel() {
		List<Order> list = orderService.findByPager(pager).getList();
		
	    Map<String, String> title = new HashMap<String, String>();
	    title.put("orderSn", "订单编号");
//	    title.put("Order.member.username", "用户名");
//	    title.put("username", "用户名");
	    title.put("shipMobile", "手机号码");
	    title.put("shipName", "收货人姓名");
	    title.put("shipArea", "配送地区");
//	    title.put("createDate", "下单时间");
	    HttpServletResponse response=ServletActionContext.getResponse();
	    
	    try {
	    	ExcelOper.export("配送单管理", list, title, ServletActionContext.getResponse());
	    } catch (Exception e) {
	    		e.printStackTrace();
	    		try {
	    			response.setContentType("text/html");
	    			response.setCharacterEncoding("utf-8");
	    			response.getWriter().write("{success:false,msg:\"导出时异常\"}");
	...
	public String edit() {
		...
		if (order.getPaymentStatus() != com.faithbj.shop.entity.Order.PaymentStatus.unpaid) {
	...
	public String update() {
		...
		if (persistent.getPaymentStatus() != com.faithbj.shop.entity.Order.PaymentStatus.unpaid) {
			addActionError("此订单付款状态无法编辑！");
			return ERROR;
		}
		...
		for (OrderItem orderItem : orderItemList) {
			...
			if (product.getStore() != null) {
				...
				if ((getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.unpaid) || 
				...
				if (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.order || (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != com.faithbj.shop.entity.Order.PaymentStatus.unpaid) || 
					(getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() != ShippingStatus.unshipped)) {
	...
	public String payment() {
		...
		if (order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.paid || order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.partRefund || order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.refunded) {
		...
		// 库存处理
		if (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) {
		...
		order.setOrderStatus(OrderStatus.processed);
		if (payment.getTotalAmount().compareTo(order.getTotalAmount().subtract(order.getPaidAmount())) == 0) {
			order.setPaymentStatus(com.faithbj.shop.entity.Order.PaymentStatus.paid);
		} else {
			order.setPaymentStatus(com.faithbj.shop.entity.Order.PaymentStatus.partPayment);
		}
		...
		for (DeliveryItem deliveryItem : deliveryItemList) {
			...
			for (OrderItem orderItem : orderItemSet) {
				if (product == orderItem.getProduct()) {
					...
					if (product.getStore() != null) {
						if ((getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) || (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() == ShippingStatus.unshipped)) {
	...
	public String refund() {
		...
		if (order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.unpaid || order.getPaymentStatus() == com.faithbj.shop.entity.Order.PaymentStatus.refunded) {
			addActionError("此订单付款状态无法支付！");
			return ERROR;
		}
		...
		order.setOrderStatus(OrderStatus.processed);
		if (refund.getTotalAmount().compareTo(order.getPaidAmount()) < 0) {
			order.setPaymentStatus(com.faithbj.shop.entity.Order.PaymentStatus.partRefund);
		} else {
			order.setPaymentStatus(com.faithbj.shop.entity.Order.PaymentStatus.refunded);
		}
	...
	public String invalid() {
		...
		if (order.getOrderStatus() == OrderStatus.completed) {
			...
			return ajaxJsonErrorMessage("此订单已经作废！");
		} else if (order.getPaymentStatus() != com.faithbj.shop.entity.Order.PaymentStatus.unpaid) {
			...
			// 库存处理
			if (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.order || (getSystemConfig().getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != com.faithbj.shop.entity.Order.PaymentStatus.unpaid) || order.getShippingStatus() != ShippingStatus.unshipped) {
				for (OrderItem orderItem : order.getOrderItemSet()) {
					Product product = orderItem.getProduct();
					product.setFreezeStore(product.getFreezeStore() - orderItem.getProductQuantity());
					if (!product.getIsOutOfStock()) {
