...
<title>添加/编辑耕种地块 - Powered By ${systemConfig.systemName}</title>
...
$().ready(function() {
	...
	function chooseSeed()
	{
		var data = window.showModalDialog('${base}/vegetableadmin/seed!popup.action?t=' + new Date().getTime(), '', 'dialogWidth:800px; dialogHeight:600px; center:yes');
		if (data)
		{
			var obj = eval('(' + data + ')');
...
<body class="input">
	<div class="body">
		<div class="inputBar">
			<h1><span class="icon">&nbsp;</span><#if isAdd??>添加耕种地块<#else>编辑耕种地块</#if></h1>
		</div>
		<div class="blank"></div>
		<form id="inputForm" name='listForm' class="validate" action="<#if isAdd??>farming_block!save.action<#else>farming_block!update.action</#if>" method="post" enctype="multipart/form-data" >
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<#if isAdd??>
							<input type="text" name="farmingBlock.code" class="formText {required: true, name: true, remote: 'farming_block!checkCode.action?oldValue=${(farmingBlock.code?url)!}', minlength: 2, maxlength: 20, messages: {remote: '耕种地块代码已存在,请重新输入!'}}" title="耕种地块代码不能为空" value="${(farmingBlock.code)!}"/>
				...
				<tr>
					...
					<td>
						<input type="text" name="farmingBlock.rendBlockCode" readonly=true class="formText {required: true, name: true, maxlength: 255}", title="承包地块代码不能为空" value="${(farmingBlock.rendBlockCode)!}"/>
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" name="farmingBlock.area" class="formText {required: true, name: true, digits:true}", title="面积不能为空" value="${(farmingBlock.area)!}"/>
							<label class="requireField">*</label>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="text" name="farmingBlock.username" class="formText {required: false, name: true, maxlength : 255}", value="${(farmingBlock.username)!}"/>
					</td>
				</tr>
				<tr>
					<th>
