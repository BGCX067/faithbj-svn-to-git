...
<body class="list">
	<div class="body">
		...
		<form id="listForm" name='listForm' action="farming_block!popup.action" method="post">
			<div class="operateBar">
				<input type="button" class="addButton" onclick="location.href='farming_block!add.action'" value="添加耕种地块" />
				<label>查找:</label>
			...
			<table class="listTable">
				...
				<#list pager.list as list>
					<tr>
						<td>
							<input type="checkbox" name="ids" value="{id:'${list.id}',code:'${list.code}',seedid:'${(list.seed.id)!}',seedname:'${(list.seed.name)!}'}" />
						</td>
						<td>
							${list.code}
						</td>
