...
$().ready( function() {
	...
	$quantity.change( function() {
		...
		//var url = type == 'product' ? "cart_item!ajaxEdit.action" : "/fieldmember/field_block_cart_item!ajaxEdit.action";
		var url = "cart_item!ajaxEdit.action" ;
	...
	$deleteCartItem.click( function() {
		...
		var type = $this.metadata().type;
		//var url = type == 'product' ? "cart_item!ajaxDelete.action" : "/fieldmember/field_block_cart_item!ajaxDelete.action";
		var url = "cart_item!ajaxDelete.action" ;
				...
				<table class="listTable">
					...
					<#list cartItemList as list>
						<tr>
							...
							<td>
								<input type="text" name="quantity" class="formText quantity {id: '${list.product.id}', type:'product'}" value="${list.quantity}">
							...
							<td>
								<a class="deleteCartItem {id: '${list.product.id}', type:'product'}" href="javascript: void(0);">删除</a>
							</td>
						</tr>
					</#list>
					
					<#list fieldBlockCartItemList as list>
						<tr>
							<td class="productName">
								<a href="${base}" target="_blank">
									<img src="${base}${(list.fieldBlock.productImageList[0].thumbnailProductImagePath)!systemConfig.defaultThumbnailProductImagePath}" />
									${list.name}
								</a>
							</td>
							<#if (loginMember.memberRank.preferentialScale != 100)!>
								<td class="priceTd">
									<span class="lineThrough">${list.preferentialPrice?string(priceCurrencyFormat)}</span>
								</td>
								<td class="priceTd">
									${list.preferentialPrice?string(priceCurrencyFormat)}
								</td>
							...
							<td>
								<input type="text" name="quantity" class="formText quantity {id: '${list.fieldBlock.id}', type:'fieldBlock'}" value="${list.quantity}">
								<#if (list.fieldBlock.store != null && list.quantity > list.fieldBlock.store)>
									<strong class="storeInfo red">[库存不足]</strong>
								</#if>
							</td>
							<td>
							...
							<td>
								<a class="deleteCartItem {id: '${list.fieldBlock.id}', type:'fieldBlock'}" href="javascript: void(0);">删除</a>
							</td>
						</tr>
					</#list>
					
					<#if (cartItemList == null || cartItemList?size == 0)! && (fieldBlockCartItemList == null || fieldBlockCartItemList?size == 0)!>
						<tr>
