...
<body class="orderInfo">
	...
	<div class="body">
		...
		<div class="orderInfoDetail">
			<form id="orderInfoForm" action="order!save.action?totalPoint=${totalPoint}" method="post">
				<@s.token />
				...
				<table class="cartItemTable">
					...
					<#list fieldBlockCartItemSet as list>
						<tr>
							<td class="productName">
								<a href="${base}" target="_blank">
									<img src="${base}${(list.fieldBlock.productImageList[0].thumbnailProductImagePath)!systemConfig.defaultThumbnailProductImagePath}" /> ${list.name}
								</a>
							</td>
							<#if (loginMember.memberRank.preferentialScale != 100)!>
								<td class="priceTd">
									<span class="lineThrough">${list.fieldBlock.rent?string(priceCurrencyFormat)}</span>
								</td>
								<td class="priceTd">
									${list.preferentialPrice?string(priceCurrencyFormat)}
								</td>
					...
					<tr>
						<td class="info" colspan="<#if (loginMember.memberRank.preferentialScale != 100)!>6<#else>5</#if>">
							...
							配送费用：
							<#if loginMember.rendBlockList.size()>0>
							<span id="deliveryFee" class="red">${0}</span>&nbsp;&nbsp;&nbsp;&nbsp;
							<#else>
							<span id="deliveryFee" class="red">${0?string(orderUnitCurrencyFormat)}</span>&nbsp;&nbsp;&nbsp;&nbsp;
							</#if>
