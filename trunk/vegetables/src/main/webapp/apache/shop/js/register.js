...
$().ready( function() {
		...
		onShow: function(hash){
			if ($.trim($("#agreementContent").html()) == "") {
				$.ajax({
					...
					url: faithbj.base + "/shop/member!getAgreement.action",
	...
	function registerWindowCaptchaImageRefresh() {
		$("#registerWindowCaptchaImage").attr("src", faithbj.base + "/captcha.jpg?timestamp" + (new Date()).valueOf());
	}
