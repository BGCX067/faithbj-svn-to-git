...
<title>新闻更新 - Powered By <#--${systemConfig.systemName}--></title>
	...
	$("#inputForm").ajaxForm({
		...
		success: function(data) {
			if (data.status == "ARTICLE_BUILDING") {
				...
				$("#status").text("正在更新新闻[" + (firstResult + 1) + " - " + (firstResult + maxResults) + "]，请稍后...");
				...
				$.message("success", "新闻更新成功！[更新总数: " + data.buildTotal + "]");
			}
		}
	});
...
<body class="input">
	<div class="body">
		<div class="inputBar">
			<h1><span class="icon">&nbsp;</span>新闻更新</h1>
		</div>
