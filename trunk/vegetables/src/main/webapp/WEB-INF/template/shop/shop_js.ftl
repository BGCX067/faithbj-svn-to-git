/***
 *	faithbj Shop JavaScript
 *
 *	http://www.faithbj.com
 *
 **/
...
$().ready( function() {
	...
	$.memberVerify = function () {
		if(getCookie("memberUsername") != null) {
			...
			$.ajax({
				url: faithbj.base + "/shop/member!ajaxMemberVerify.action",
				type: "POST",
				dataType: "json",
				async: false,
				cache: false,
	...
	if ($header.size() > 0) {
		...
		$.showLoginWindow = function (redirectUrl) {
					...
					<table>
						...
						<tr>
							...
							<td>
								...
								<img id="loginWindowCaptchaImage" class="loginWindowCaptchaImage" src="' + faithbj.base + '/captcha.jpeg?timestamp=' + (new Date()).valueOf() + '" alt="换一张" />
							</td>
						</tr>
							<tr>
								<th>&nbsp;</th>
							<td>
								...
								<a href="' + faithbj.base + '/shop/member!passwordRecover.action">忘记了密码? 点击找回!</a>
							</td>
						</tr>
					</table>
				</form>';
			...
			function login() {
				...
				$.ajax({
					url: faithbj.base + "/shop/member!ajaxLogin.action",
		...
		$.showRegisterWindow = function () {
					...
					<table>
						...
						<tr>
							...
							<td>
								...
								<img id="registerWindowCaptchaImage" class="registerWindowCaptchaImage" src="' + faithbj.base + '/captcha.jpeg?timestamp=' + (new Date()).valueOf() + '" alt="换一张" />
							</td>
						</tr>
						<tr>
							<th>&nbsp;</th>
			...
			function register() {
				...
				$.ajax({
					url: faithbj.base + "/shop/member!checkUsername.action",
					...
					success: function(data) {
						if (data == "true") {
							$.ajax({
								url: faithbj.base + "/shop/member!ajaxRegister.action",
	...
	$.addFavorite = function(id) {
		...
		$.ajax({
			url: faithbj.base + "/shop/favorite!ajaxAdd.action",
			data: {"id": id},
			type: "POST",
			dataType: "json",
			cache: false,
	...
	$.addCartItem = function(id, quantity) {
		...
		$.ajax({
			url: faithbj.base + "/shop/cart_item!ajaxAdd.action",
								...
								<#list instantMessagingList as instantMessaging>
									<dd>
										<#if instantMessaging.instantMessagingType == "qq">
											<a href="http://wpa.qq.com/msgrd?v=3&uin=${instantMessaging.value}&site=faithbj&menu=yes" target="_blank">
										...
										<#elseif instantMessaging.instantMessagingType == "msn">
											<a href="msnim:chat?contact=${instantMessaging.value}" target="_blank">
												<img src="${base}/vegetables/shop/images/instant_messaging_msn.gif" alt="MSN在线客服" align="absmiddle" /> ${instantMessaging.title}
											</a>
										<#elseif instantMessaging.instantMessagingType == "wangwang">
											<a href="http://amos1.taobao.com/msg.ww?v=2&uid=${instantMessaging.value}&s=2" target="_blank">
												<img src="${base}/vegetables/shop/images/instant_messaging_wangwang.gif" alt="旺旺在线客服" align="absmiddle" /> ${instantMessaging.title}
											</a>
										<#elseif instantMessaging.instantMessagingType == "skype">
											<a href="callto://${instantMessaging.value}" target="_blank">
												<img src="${base}/vegetables/shop/images/instant_messaging_skype.gif" alt="Skype在线客服" align="absmiddle" /> ${instantMessaging.title}
											</a>
										</#if>
									</dd>
								</#list>
