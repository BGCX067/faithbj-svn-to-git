...
<title>${(pager.keyword)!} 文章搜索结果<#if setting.isShowPoweredInfo> - Powered By faithbj</#if></title>
						...
						<@article_list type="recommend" count=10; articleList>
							<#list articleList as article>
								<li>
									...
									<a href="${base}${article.htmlPath}" title="${article.title}"><#if article.title?length lt 24>${article.title}<#else>${article.title[0..24]}...</#if></a>
								</li>
							</#list>
						</@article_list>
					</ul>
