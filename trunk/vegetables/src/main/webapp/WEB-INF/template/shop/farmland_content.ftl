...
<title>土地详情 - Powered By faithbj</title>
...
<body id="goodsContent" class="goodsContent">
	...
	<div class="body">
					...
					<ul>
						<#list goodsList as goods>
							<li class="number${goods_index + 1}">
								...
								<a href="${base}/product/${goods.id}" title="${goods.name}"><#if goods.name?length lt 24>${goods.name}<#else>${goods.name[0..24]}...</#if></a>
							</li>
						</#list>
					</ul>
				</div>
		...
		<div class="bodyRight">
			...
			<div class="goodsTop">
					...
					<div class="thumbnailGoodsImage">
						...
						<div id="thumbnailGoodsImageScrollable" class="scrollable">
							<ul id="goodsImageTab" class="items goodsImageTab">
								...
								<#else>
									<li><img src="${base}/vegetables/inventory/${setting.defaultThumbnailGoodsImagePath}" /></li>
	                        	</#if>
							</ul>
						</div>
				...
				<div class="goodsTopRight">
					...
					<table id="buyInfo" class="buyInfo">
					   <#if setting.pointType == "productSet" && goods.point != 0>
							<tr>
								<th>所得积分:</th>
								<td>
			...
			<div class="goodsBottom">
				...
				<div class="detailtab tabContent goodsIntroduction">
					${goods.introduction}
				</div>
				<div class="detailtab tabContent goodsAttribute">
					<table class="goodsParameterTable">
						<#list (goods.productType.productParameters)! as goodsParameter>
							<#if goods.getProductParameterMapValue(goodsParameter.id)?? && goods.getProductParameterMapValue(goodsParameter.id) != "">
								<tr>
									<th>
										${goodsParameter.name}
									</th>
									<td>
										${(goods.getProductParameterMapValue(goodsParameter.id))!}
									</td>
								</tr>
							</#if>
				       </#list>
				...
				<#if setting.isCommentEnabled>
					<div id="comment" class="detailtab tabContent comment">
	...
	$().ready( function() {
		...
		$goodsButton.click(function () {
			$.addLandCartItem(selectedProductId);
		});
