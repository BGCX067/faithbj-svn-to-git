...
<title>查看购物车 - Powered By faithbj</title>
...
$().ready( function() {
		...
		$.ajax({
			...
			success: function(data) {
				if (data.status == "success") {
					...
					$totalProductQuantity.text(data.totalQuantity);
					$totalProductPrice.text(currencyFormat(data.totalPrice));
					$totalScore.text(currencyFormat(data.totalPoint));
					$changeQuantityTipTopMessage.text("商品数量修改成功!");
					$changeQuantityTipBottomMessage.text("商品总金额: " + currencyFormat(data.totalPrice));
					$changeQuantityTip.fadeIn();
					setTimeout(function() {$changeQuantityTip.fadeOut()}, 3000);
				} else {
					var previousQuantity = $this.data("previousQuantity");
	...
	$deleteCartItem.click( function() {
		...
		function deleteCartItem() {
			$.ajax({
				url: "/product/caritem/ajaxDelete",
				data: {"id": productId},
				type: "POST",
				dataType: "json",
				cache: false,
	...
	$orderInfoButton.click( function() {
		...
		if (!$.memberVerify()) {
			$.showLoginWindow("/cjlhome/order/info");
			return false; 
		}
	});
			...
			<div class="middle">
				<table id="cartItemTable" class="listTable">
					...
					<#list cartItemList as cartItem>
						<#assign goods = cartItem.product />
						<tr>
							<td class="goodsImage">
								<#if goods.productImageList??>
									<li><img src="${base}/vegetables/inventory/${goods.productImageList[0].thumbnailImageView}" alt="${goods.productImageList[0].description}" /></li>
								<#else>
									<li><img src="${base}/vegetables/shop/images/${setting.defaultThumbnailProductImagePath}"/></li>
	                        	</#if>
							</td>
							<td>
								<a href="${base}/product/${goods.id}" target="_blank">
									${goods.name}
								</a>
							</td>
							<#if (loginMember.memberRank.preferentialScale != 100)!>
								<td class="priceTd">
									<span class="lineThrough">${goods.price?string(currencyFormat)}</span>
								</td>
								<td class="priceTd">
							...
							<td>
								<input type="text" name="quantity" class="formText quantity" value="${cartItem.quantity}" productId="${goods.id}" />
								<#if (goods.store?? && goods.freezeStore + cartItem.quantity > goods.store)>
							...
							<td>
								<a class="deleteCartItem" href="javascript: void(0);" productId="${goods.id}">删除</a>
							</td>
						</tr>
					</#list>
					...
					<#else>
						<tr>
							<td class="info" colspan="<#if (loginMember.memberRank.preferentialScale != 100)!>7<#else>6</#if>">
								商品共计: <span id="totalProductQuantity" class="red">${totalQuantity}</span> 件&nbsp;&nbsp;
								<#if setting.pointType != "disable">
									积分: <span id="totalScore" class="red">${totalPoint}</span>&nbsp;&nbsp;
								</#if>
								商品总金额(不含运费): <span id="totalPrice" class="red">${totalPrice?string(currencyFormat)}</span>
							</td>
						</tr>
					</#if>
				</table>
				...
				<#if (cartItemList?? && cartItemList?size > 0)!>
					...
					<a id="orderInfoButton" class="formButton" href="${base}/cjlhome/order/info">去结算</a>
				</#if>
			</div>
			<div class="bottom">
